...
bot = telebot.TeleBot('7678896440:AAFzudQDRxK4f5sX0qZ0e87uR1DJTp6XjLM')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "🤡 𝗢𝗡𝗟𝗬 𝗣𝗔𝗣𝗔 𝗖𝗔𝗡 𝗨𝗦𝗘 𝗧𝗛𝗜𝗦 𝗖𝗢𝗠𝗠𝗔𝗡𝗗 🤡"
...
# Function to handle the reply when free users run the /vansh command
...
# Dictionary to store the last time each user ran the /vansh command
vansh_cooldown = {}
...
def handle_vansh(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in vansh_cooldown and (datetime.datetime.now() - vansh_cooldown[user_id]).seconds < 50:
                response = "❕𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗶𝗻 𝗲𝗳𝗳𝗲𝗰𝘁 𝗽𝗹𝗲𝗮𝘀𝗲 𝘄𝗮𝗶𝘁 50 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❕"
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            vansh_cooldown[user_id] = datetime.datetime.now()
...
# Add /mylogs command to display logs recorded for vansh and website commands
...
def welcome_start(message):
    ...
    response = f"🔆 𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗖𝗫𝗧 𝗗𝗗𝗢𝗦 𝗕𝗢𝗧 𝟮𝟰/𝟳 🔆"
    bot.reply_to(message, response)
